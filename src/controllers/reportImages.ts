  ...
  try {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/api/face/embedding",
        formData,
        {
      ...
      if (
        ...
        Array.isArray(response.data.vector) &&
        response.data.vector.length === 128
      ) {
        hasFace = true;
...
export const updateImage = async (req: Request, res: Response) => {
  try {
    ...
    const newImageUrl = await updateCloudinaryImage(
...
export const findMatchingFace = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      res
        .status(500)
        .json({ success: false, message: "Invalid embedding received" });
      return;
    }
